...
def main() -> int:
    parser = argparse.ArgumentParser(description="Capture before/after screenshots around a single click.")
    parser.add_argument("--client-x", type=int, required=True)
    parser.add_argument("--client-y", type=int, required=True)
    ...
    before_path = output_root / f"{args.label}-before.png"
    before.image.save(before_path)

    screen_x = info.client_rect.left + args.client_x
    screen_y = info.client_rect.top + args.client_y
    send_count, last_error = _send_mouse_click(screen_x, screen_y)

    time.sleep(args.delay)
    after = _capture_window_bitblt(info.handle)
    after_path = output_root / f"{args.label}-after.png"
    ...
    result = ProbeResult(
        ...
        screen_x=screen_x,
        screen_y=screen_y,
        client_x=args.client_x,
        client_y=args.client_y,
